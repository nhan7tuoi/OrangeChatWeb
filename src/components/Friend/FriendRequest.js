...
import { addFriendRequests, deleteFriendRequest, fetchFriendRequests } from '../../redux/friendSilce';
// import connectSocket from './../../server/connectSocket';
    ...
    return (
        ...
        <Row style={{ width: '100vw', height: '90vh', background: '#242424' }}>
            ...
            <Col span={18} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', border: '1px solid #2E2E2E' }}>
                <header style={{ fontSize: '32px', fontWeight: '700', color: '#FFF', arginLeft: '30px', marginTop: '30px', marginBottom: '20px' }}>Lời mời kết bạn</header>
                {Array.isArray(listFriendRequests) && listFriendRequests.map((friends, index) => (
                    <div key={index} style={{ display: 'flex', width: '90%', background: '#242424', padding: '20px', alignItems: 'center', justifyContent: 'space-between', marginTop: '10px', marginLeft: '30px', border: '2px solid #2E2E2E', borderRadius: '10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center' }}>
                            <img src={friends.senderId.image} style={{ height: '50px', width: '50px', borderRadius: '50%' }} alt='image'></img>
                            <Text style={{ marginLeft: '20px', color: 'white', fontSize: '20px' }}>{friends.senderId.name}</Text>
                        </div>
                        <div style={{ display: 'flex' }}>
                            <Button style={{ display: 'flex', width: '50px', height: '50px', background: '#36373A', borderRadius: '100%', justifyContent: 'center', alignItems: 'center', border: 'hidden' }}
                                onClick={() => {
                                    ...
                                    dispatch(deleteFriendRequest(friends.senderId._id));
                                }}
                            >
                            ...
                            <Button style={{ display: 'flex', width: '50px', height: '50px', background: '#36373A', borderRadius: '100%', justifyContent: 'center', alignItems: 'center', border: 'hidden', marginLeft: '10px' }}
                                onClick={() => {
                                    ...
                                    connectSocket.emit('create new conversation', {
                                        nameGroup: '',
                                        isGroup: false,
                                        members: [user._id, friends.senderId._id],
                                    });
